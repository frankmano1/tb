...
bot.on('message', (msg) => {
  ...
  switch (text) {
    ...
    case 'Ricevi le nostre news':
    case 'Chatta con l\'AI':
      mainMenuOptions.forEach((option) => {
        if (text === option.text) {
          if (option.action === 'webview') {
            const webviewOptions = {
              reply_markup: {
                inline_keyboard: [{ text: 'Apri', url: option.url }],
              },
            };
            bot.sendMessage(chatId, `Hai selezionato ${option.text}. Clicca sul pulsante per aprire la webview.`, webviewOptions);
          } else if (option.action === 'action') {
            // Gestisci l'azione specifica qui
            bot.sendMessage(chatId, `Hai selezionato ${option.text}.`);
          } else {
